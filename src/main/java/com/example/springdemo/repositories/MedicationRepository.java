...
public interface MedicationRepository extends JpaRepository<Medication, Integer> {
    ...
    @Query(value = "select u from Medication u where u.id_patient = ?1")
    ...
    @Query(value = "update Medication u set u.name = ?2, u.start = ?3, u.end = ?4, u.sideEffects=?5, u.dosage = ?6, u.id_patient = ?7 where u.medication_id = ?1")
